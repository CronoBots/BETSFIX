...
def _drop_iproyal() -> bool:
    # ANCRE SHARP DÉCOUPLÉE de SofaScore (user 2026-09-23 « rétablir l'ancienne logique ») : iProyal est
    # RÉ-ACTIVÉ pour l'ancre Pinnacle via un flag DÉDIÉ `BETSFIX_DROP_IPROYAL_SHARP` (défaut 0 = iProyal ON),
    # SANS toucher le flag partagé `drop_iproyal()` qui garde SofaScore (sofa_http/browser) HORS proxy.
    # Réversible : BETSFIX_DROP_IPROYAL_SHARP=1 recoupe l'ancre iProyal → délégué API-Football.
    return os.environ.get("BETSFIX_DROP_IPROYAL_SHARP", "0").strip().lower() not in ("0", "false", "no", "off")
