...
COMBO_MISSION = (
    ...
    "   • Corners TOTAUX du match over AVEC un nombre : « Total des corners plus de 7.5 » (les corners "
    "TOTAUX over sont fiables chez nous). ⛔ ÉVITE les corners PAR ÉQUIPE (« <équipe> plus de 4.5 corners ») "
    "et les HANDICAPS corners (lignes serrées) : DATA INTERNE = LE marché le plus perdant de nos combinés "
    "(~58 %, à égalité avec les cartons). JAMAIS « le plus de corners » sans chiffre (non réglable).\n"
    ...
    "   ⛔ N'utilise PAS : « premier but / premier buteur », « gagne une mi-temps / mi-temps » (non "
    "réglables — restent « en attente »), les CARTONS (règle 1), les corners PAR ÉQUIPE / handicaps "
    "corners, ni les TIRS TOTAUX (« Total de tirs +X.5 », « <équipe> plus de 22.5 tirs » : compte exact "
    "imprévisible, 0/2 chez nous). Les « tirs CADRÉS » avec un nombre RESTENT autorisés. Reformule toute "
    "autre idée vers un marché réglable ET fiable ci-dessus.\n"
...
METHODO = (
    ...
    "L'<explication> = la JUSTIFICATION la PLUS précise et PROFESSIONNELLE de CE pari, en 3 à 5 phrases "
    "COURTES et AUTONOMES (le site les affiche EN PUCES, une par phrase — donc PAS un pavé : chaque phrase "
    "porte UN seul argument, va droit au but, et se lit seule). Couvre, dans cet ordre : (1) le SCÉNARIO de "
    "match / la forme récente CHIFFRÉE et les absents déterminants ; (2) les STATS clés par équipe/joueur "
    "(H2H, moyennes, contexte/enjeu, surface/repos) ; (3) la VALUE (ta proba estimée vs proba juste de la "
    "cote, ex. « ~72 % vs 64 % juste → value nette ») ; (4) le RISQUE principal à connaître. Ton de pro du "
    "pari sportif, chiffres concrets, français impeccable, ZÉRO généralité ni remplissage ni redite.\n"
