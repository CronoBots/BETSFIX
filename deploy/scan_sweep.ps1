# BETSFIX — SWEEP « analyse ~2 h avant le coup d'envoi » (tâche « BETSFIX Scan Sweep », compte vince).
#
# MODE WAVE-FIRST (user 2026-08-11) : au lieu d'analyser tout le slate en batch le matin (un match de 20h
# analysé à 10h = 10 h de retard, sans compos), CHAQUE match est analysé ~2 h avant SON coup d'envoi, avec
# des cotes mûres. UNE seule analyse (garde-fou code `_generated_today` -> match déjà analysé sauté =>
# AUCUN flip du pick). Le matin ne fait plus que SÉLECTIONNER (écrire le programme).
#
# RÔLE de ce sweep (toutes les ~30 min) :
#   1) ANALYSE les matchs du PROGRAMME dont le coup d'envoi tombe dans les 2,5 h à venir et qui ne sont pas
#      encore analysés (1re et seule analyse). Fenêtre 2,5 h + grille 30 min => chaque match est pris ~2 h
...
# --top 16 : ne jamais écrêter un match du programme présent dans la fenêtre (une fenêtre 2,5 h a peu de matchs).
